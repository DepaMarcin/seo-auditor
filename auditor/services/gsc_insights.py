...
def _period_suffix(period_label: str) -> str:
    """Dopisek o analizowanym okresie - pusty, gdy zakres nie został podany."""
    return f" w okresie {period_label}" if period_label else " w analizowanym okresie"
...
def generate_query_commentary(result: dict, period_label: str = "") -> str:
    ...
    if not gainers and not losers:
        return f"Brak istotnych zmian w kliknięciach dla poszczególnych fraz{_period_suffix(period_label)}."
    ...
    if gainers:
        ...
        parts.append(
            ...
            f'(+{top["delta"]} kliknięć){_format_extras(gainers, "query")}{_period_suffix(period_label)}.'
        )
    if losers:
        top = losers[0]
        parts.append(
...
def generate_page_commentary(result: dict, period_label: str = "") -> str:
    ...
    if not gainers and not losers:
        return f"Brak istotnych zmian w ruchu na poszczególnych podstronach{_period_suffix(period_label)}."
    ...
    if gainers:
        ...
        parts.append(
            ...
            f'(+{top["delta"]} kliknięć){_format_extras(gainers, "page")}{_period_suffix(period_label)}.'
        )
    if losers:
        top = losers[0]
        parts.append(
